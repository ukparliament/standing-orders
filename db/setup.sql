...
create table adoptions (
	id serial,
	adoption_date_id int not null,
	standing_order_fragment_version_id int not null,
	standing_order_number varchar(5) not null,
	standing_order_number_in_list int,
	standing_order_letter_in_list char(1),
	fragment_number_in_list int not null,
	constraint fk_adoption_adoption_date foreign key (adoption_date_id) references adoption_dates(id),
	constraint fk_standing_order_fragment_version foreign key (standing_order_fragment_version_id) references standing_order_fragment_versions(id),
	primary key (id)
);